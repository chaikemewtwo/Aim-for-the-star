...
void Map::Update() {
	...
	for (int i = 0; i < 2; i++) {
		...
		{
			// 移動位置取得
			player_move[i] = m_p_base[i]->GetMove();

			// 自機の位置を代入,当たりポイントを補正
			player_pos[i].x = m_p_base[i]->GetPos().x + HIT_VERTEX_X;
			player_pos[i].y = m_p_base[i]->GetPos().y + HIT_VERTEX_Y;
		}
		...
		// 自機(obj)の位置変更
		m_p_base[i]->SetPos(player_pos[i]);
		// 自機の移動ベクトル変更
		m_p_base[i]->SetMove(player_move[i]);
	}
