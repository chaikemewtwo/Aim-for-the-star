...
namespace Texture {
	...
	void Draw2D(const char*file_name,float x,float y, float scale_w,float scale_h,float angle,float cx,float cy,bool uv_cut,int u_axis,int v_axis,int graph_num) {
		...
		CUSTOM_VERTEX cv[4] =
		{
		{ x1,y1,0.0f,1.0f,D3DXCOLOR(255.f,255.f,255.f,1.5f),up_left->x,up_left->y },       // 左上
		{ x2,y1,0.0f,1.0f,D3DXCOLOR(255.f,255.f,255.f,1.5f),up_right->x,up_right->y },     // 右上
		{ x2,y2,0.0f,1.0f,D3DXCOLOR(255.f,255.f,255.f,1.5f),down_right->x,down_right->y }, // 右下
		{ x1,y2,0.0f,1.0f,D3DXCOLOR(255.f,255.f,255.f,1.5f),down_left->x,down_left->y },   // 左下
		};
