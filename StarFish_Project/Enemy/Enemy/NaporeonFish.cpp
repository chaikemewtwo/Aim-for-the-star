...
StateId NaporeonFish::CheckChangeState() {
	...
	if (CheckTargetInTheRange(m_pos, vec1, vec2, m_p_player_manager->GetPosRelay(Player::STAR_1)) == true &&
		CheckTargetInTheRange(m_pos, vec1, vec2, m_p_player_manager->GetPosRelay(Player::STAR_2)) == true) {
		...
		if (distance1.x < distance2.x) {
			...
			m_anim_change_time = 15;
			m_speed = 3.f;
			m_target_pos = m_p_player_manager->GetPosRelay(Player::STAR_1);
			...
			m_anim_change_time = 15;
			m_speed = 3.f;
			m_target_pos = m_p_player_manager->GetPosRelay(Player::STAR_2);
	...
	// プレイヤー１が範囲内のとき
	else if (CheckTargetInTheRange(m_pos, vec1, vec2, m_p_player_manager->GetPosRelay(Player::STAR_1)) == true) {
	...
	// プレイヤー２が範囲内のとき
	else if (CheckTargetInTheRange(m_pos, vec1, vec2, m_p_player_manager->GetPosRelay(Player::STAR_2)) == true) {
